...
User* First_step(User* root) {
    ...
    if (!file) {
        ...
        while (fgets(buffer,
            ...
            /*if (row == 1)
                continue;*/ //Doesnt read first row
...
int Show_menu(){
    ...
    printf("6. Find data\n");
    printf("7. Exit\n");
    printf("Input action [1-7]:\n");
    int actionId;
    scanf("%d", &actionId);
    boolean exp = actionId == 1 || actionId == 2 || actionId == 3 || actionId == 4 || actionId == 5 || actionId == 6 || actionId == 7;
    if(exp==false){
        printf("Input correct action [1-7]:\n");
    }
    return actionId;
}
...
int Rewrite_file(User* root) {
    ...
    int main() {
        ...
        //User *sortUser = NULL;
        //memcpy (sortUser, rootUser, sizeof (*sortUser));
        //sortUser->name = strdup (rootUser->name);
        ...
        while (t) {
            ...
            switch (action) {
                ...
                case 4: {
                    int sort_flag = 0;
                    printf("Sort parameters: 1 - inc, 2 - desc: \n");
                    scanf("%d", &sort_flag);

                    if (sort_flag == 1) {
                        rootUser = Sort_salary_inc(rootUser);
                        //sortUser = Sort_salary_inc(rootUser);
                    } else if (sort_flag == 2) {
                        rootUser = Sort_salary_desc(rootUser);
                       // sortUser = Sort_salary_inc(rootUser);
                    }
                    List_show(rootUser);
                    //List_show(sortUser);
                ...
                case 5:{
                    ...
                    if (sort_flag == 1) {
                        rootUser = Sort_names_inc(rootUser);
                    }
                    else if (sort_flag == 2) {
                        rootUser = Sort_names_desc(rootUser);
                    }
                    List_show(rootUser);
